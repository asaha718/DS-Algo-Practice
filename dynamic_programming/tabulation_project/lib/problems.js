...
// function stepper(nums) { //tabulation method
//     let table= new Array(nums.length).fill(false); //we want to create a new table that has a position for each num 
//     table[0]= true; // initulize

//     for(let i=0; i <= table.length; i++){ 
//         if(table[i]===true){ // only if that value holds 
//             for(j=1; j<=nums[i]; j++){ 
//                 table[i+j]= true; //change values on table  
//             }
//         }
//     }
...
// }
function stepper(nums, obj={}) { //memoization method
    if( nums.length in obj) return obj[nums.length]; 
    if(nums.length===0) return true; 
    let maxStep= nums[0]; 

    for(let i=1; i<= maxStep; i++){ 
        if (stepper(nums.slice(i), obj)){ 
            obj[nums.length]= true; 
            return true; 
        }
    }
    obj[nums.length]= false; 
    return false
